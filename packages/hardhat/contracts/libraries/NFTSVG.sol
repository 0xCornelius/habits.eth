...
library NFTSVG {
  ...
  function generateSVG(HabitSVGParams memory params) public pure returns (string memory svg) {
    uint256 _percentage = (params.chainCommitment > 0) ? ( (params.chain * 100) / params.chainCommitment ) : 100;
    return
      string(
        abi.encodePacked(
          ...
          _generateSVGCardTitle(params.name, params.timesPerTimeframe, params.timeframeString),
          _generateSVGPositionData(params.habitId, params.stake, params.beneficiary),
          _generateSVGBorderText(params.beneficiary),
          _generateSVGLinesAndMainLogo(_percentage),
          _generageSVGProgressArea(params.chain, params.chainCommitment),
          '</svg>'
        )
      );
  }

  function _generateStyleDefs(uint256 _percentage) private pure returns (string memory svg) {
    svg = string(
      abi.encodePacked(
  ...
  function _generateSVGBorderText(
    ...
    string memory _fromText = "Beneficiary";
    string memory _toText = _beneficiary;
  ...
  function _generageSVGProgressArea(uint256 _chain, uint256 _chainCommitment) private pure returns (string memory svg) {
    svg = string(
      abi.encodePacked(
        '<text text-rendering="optimizeSpeed"><textPath xlink:href="#text-path-executed"><tspan class="st38 st58" fill="#d1dbe0" style="text-shadow:#214c64 0px 0px 5px">Done: ',
        _chain.toString(),
        '</tspan></textPath></text><text text-rendering="optimizeSpeed"><textPath xlink:href="#text-path-left" startOffset="30%" ><tspan class="st38 st58" alignment-baseline="hanging" fill="#153041" stroke="#000" stroke-width="0.5">Remaining: ',
  ...
  function _generateSVGLinesAndMainLogo(uint256 _percentage) private pure returns (string memory svg) {
    svg = string(
      abi.encodePacked(
